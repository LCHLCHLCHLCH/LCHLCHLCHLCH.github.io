...
同一时期，借助 HTML 版魔塔 51 层地图文件（来源：https://h5mota.com/tower/?name=51），补齐了 50 层的地形、门、宝石、血瓶、楼梯及大部分怪物，并实现了最初的简易背包功能。
...
在 SDL3 版本中引入了 Lua，用于地图与事件的定义。此后经过较长周期的事件编写，游戏基本完成，包含第 49 层的假魔王封印机制与第 50 层的剧情反转，以及通关流程。
...
第 49 层实现了假魔王封印机制：当正方向四守卫全部被击杀、且对角四守卫仍存活时，假魔王从 8000 血的形态转变为封印形态（地块 `134`）。该逻辑以 Lua 坐标判断实现。

## 战斗与机制

战斗采用确定性规则，不包含随机数。回合制伤害计算为双方互相扣除"攻击减去对方防御"的差值，先归零者败；攻击力不大于对方防御时判定为无法攻击。部分怪物具有先攻属性，战斗开始时先行动一次，如骑士队长及其 12 个手下。
...
多格 Boss 占据多个地块（巨型章鱼位于第 15 层，魔龙位于第 35 层），触碰任意身躯格即进入战斗，击败后身躯消失并开启后续通道。
...
CI 使用 GitHub Actions：推送 tag 时触发 MinGW 构建，打包 EXE、DLL、字体与地图为 zip 并发布 Release。git log 中记录了从 V0.0.2 至 v1.0.0 的 15 个版本。
